...
void ClientSession::SessionReset()
{
	...
	//send에 보낼 데이터가 남아있든 어떻든 그냥 Timewait상태 만들지 않기위해서 ligner option 을 이용한 Time_wait상태 만들지 않음
	/// no TCP TIME_WAIT
...
bool ClientSession::PostAccept()
{
	...
	//Question 이것도 PreRecv와 비슷한 맥락으로 볼 수 있을까?
...
{
	//실제로 데이터 받아야한다(제로바이트를 통해 연결이 되었으면 실제로 데이터 송수신을 해야함)
	if (!IsConnected())
		return false;
